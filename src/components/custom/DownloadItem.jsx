...
export default function DownloadItem({ id, data, pauseDownload }) {
			...
			>
				{/* <button
					...
					onClick={() => {
						if (data.status === 1 || data.status === 0) {
							if (data.status === 1) pauseDownload(data.id)
							setPaused((i) => !i)
						}
					}}
				>
					{data.status === -1 ? (
						...
						<FaUndo className={"size-8 scale-90"} />
					) : paused ? (
				...
				>
					<span className='block'>
						...
						{data.files?.length > 1 && <span className='opacity-50'> + {data.files.length - 1} more</span>}
					</span>
					{data.status === -1 ? (
						<Progress loading={true} className='my-2.5' />
					) : data.status !== 2 ? (
						<Progress value={downloadPercentage} className='my-2.5' />
					) : null}
						...
						<div className='flex-1'>
							{data.status === -1 ? (
								...
								<div className='flex items-center gap-2'>
									<MdOutlineErrorOutline className='fill-red-500 size-5' />
									<span className='line-clamp-2'>{data.error}</span>
								</div>
							) : null}
						</div>

{/* [-1, 0, 1].includes(data.status) ? (
							<Button
